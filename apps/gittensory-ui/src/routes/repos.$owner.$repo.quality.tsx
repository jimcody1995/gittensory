...
export const Route = createFileRoute("/repos/$owner/$repo/quality")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "Public, opt-in review-quality metrics for a repository: gate precision, merge outcomes, and weekly trend.",
      },
  ...
  component: PublicRepoQualityPage,
});

type PublicQualityMetrics = {
  repoFullName: string;
  generatedAt: string;
  gate: {
    blocked: number;
...
function PublicRepoQualityPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const result = await apiFetch<PublicQualityMetrics>(
        ...
        { label: "Public quality metrics", timeoutMs: 8000, silentStatus: true },
      );
    ...
    return (
      <Section className="pt-16 pb-16">
        ...
        <p className="mt-4 max-w-2xl text-muted-foreground">
          This repository has not opted in to public review-quality metrics, or the metrics are temporarily unavailable.
        </p>
      </Section>
    );
  }
  ...
  return (
    <Section className="pt-16 pb-16">
      <div className="max-w-4xl">
        ...
        <h1 className="mt-4 text-token-2xl font-medium tracking-tight text-foreground">{data.repoFullName}</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          Aggregate counts only — no raw trust scores, rewards, or contributor rankings. Updated {new Date(data.generatedAt).toLocaleString()}.
        </p>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          ...
          <Card className="p-5">
            ...
            <div className="mt-2 text-token-xl font-medium">
              {data.outcomes.mergeRatioPct != null ? `${pctFmt.format(data.outcomes.mergeRatioPct)}%` : "—"}
            </div>
            <p className="mt-2 text-sm text-muted-foreground">
          ...
          <Card className="p-5">
            ...
            <div className="mt-2 text-token-xl font-medium">
              {data.slop.overallMergeRate != null ? `${pctFmt.format(data.slop.overallMergeRate)}% merge` : "—"}
            </div>
            <p className="mt-2 text-sm text-muted-foreground">
              {data.slop.totalResolved} resolved PRs
              {data.slop.discriminates === true ? " · discriminating" : data.slop.discriminates === false ? " · recalibrate" : ""}
            </p>
          </Card>
        </div>
          ...
          <div className="mt-10">
            ...
            <ul className="mt-4 space-y-2 text-sm">
              {data.gate.topGateTypes.map((row) => (
                <li key={row.gateType} className="rounded-md border border-border px-4 py-3">
                  <span className="font-mono text-foreground">{row.gateType}</span>
                  <span className="text-muted-foreground">
                    {" "}
                    — {row.blocked} blocks, {row.blockedThenMerged} merged anyway
                    {row.precisionPct != null ? ` (${pctFmt.format(row.precisionPct)}% precision)` : ""}
                  </span>
                </li>
              ))}
            </ul>
                ...
                {data.trend.map((row) => (
                  <tr key={row.weekStart} className="border-t border-border">
                    ...
                    <td className="py-2 pr-4">
                      {row.gateFalsePositiveRate != null ? `${pctFmt.format(row.gateFalsePositiveRate * 100)}%` : "—"}
                    </td>
                    <td className="py-2 pr-4">
