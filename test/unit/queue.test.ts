...
describe("queue processors", () => {
  ...
  describe("force-fresh-rebase-before-merge gate (#2552)", () => {
    ...
    function stubFreshRebaseFetch(prNumber: number, opts: { baseAdvancedAt: string | null; mergeableState?: string }, seen: { merged: boolean; updateBranchCalls: number; baseCommitCalls: number }) {
      vi.stubGlobal("fetch", async (input: RequestInfo | URL, init?: RequestInit) => {
        const url = input.toString();
        const method = (init?.method ?? "GET").toUpperCase();
        if (url === "https://api.gittensor.io/miners") return Response.json([]);
        ...
        if (/\/pulls\/\d+(?:\?|$)/.test(url) && method === "GET" && !url.includes(`/pulls/${prNumber}/`)) {
          return Response.json({ number: prNumber, state: "open", user: { login: "contributor" }, head: { sha: "sha1" }, base: { ref: "main", sha: "base" }, mergeable_state: opts.mergeableState ?? "clean", labels: [] });
        }
    ...
    it("falls through to a normal merge once the bounded-retry cap is reached, with a cap-exceeded audit event", async () => {
      ...
      await env.SELFHOST_TRANSIENT_CACHE?.set("fresh-rebase-forced:owner/repo#93:sha1", "3", 24 * 3600);
      const seen = { merged: false, updateBranchCalls: 0, baseCommitCalls: 0 };
