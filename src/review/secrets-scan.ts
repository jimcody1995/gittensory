...
/** True for an obvious non-secret filler value: a known placeholder phrase, or a string built from at most 2
 *  distinct characters (e.g. "xxxxxxxxxxxxxxxx", "----------------") — real high-entropy secrets never look
 *  like either. */
function isPlaceholderSecretValue(value: string): boolean {
  if (PLACEHOLDER_VALUE_PATTERN.test(value)) return true;
  return new Set(value.toLowerCase()).size <= 2;
}
